...
class VelControllerNode : public rclcpp::Node
{
  public:
    ...
    void InitializeParams()
    {
        ...
        this->get_parameter("PWM_MP_sat_max", PWM_MP_sat_min);
        this->get_parameter("PWM_MP_sat_min", PWM_MP_sat_max);
    ...
    void CalculateVelControl()
    {
        ...
        //Conversion in PWM
        speed_motor1 = (sqrt(abs(F1)/Kd_m) * sign(F1)) * (PWM_max-PWM_0)/rot_max_m + PWM_0;
        speed_motor2 = (sqrt(abs(F2)/Kd_m) * sign(F2)) * (PWM_max-PWM_0)/rot_max_m + PWM_0;
        speed_motor3 = (sqrt(abs(F3)/Kd_m) * sign(F3)) * (PWM_max-PWM_0)/rot_max_m + PWM_0;
        speed_motor4 = (sqrt(abs(F4)/Kd_m) * sign(F4)) * (PWM_max-PWM_0)/rot_max_m + PWM_0;
        speed_motor5 = (sqrt(abs(F5)/Kd_m) * sign(F5)) * (PWM_max-PWM_0)/rot_max_d + PWM_0;
        speed_motor6 = (sqrt(abs(F6)/Kd_m) * sign(F6)) * (PWM_max-PWM_0)/rot_max_d + PWM_0;
        speed_motor7 = (sqrt(abs(F7)/Kd_m) * sign(F7)) * (PWM_max-PWM_0)/rot_max_d + PWM_0;
        speed_motor8 = (sqrt(abs(F8)/Kd_m) * sign(F8)) * (PWM_max-PWM_0)/rot_max_d + PWM_0;
